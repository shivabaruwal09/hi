...
module.exports = {
 ...
 onStart: async function({ event, api, args , message }) {
		...
		const response = await axios.get(`https://sandip-api.onrender.com/wanted?uid=${id}`, { responseType: 'stream' });
		...
		writer.on('finish', async () => {
			...
			await api.sendMessage({ body: "wanted😤", attachment: attachment }, threadID, messageID);
